...
parser.add_argument(
    "-i", "--input-index", type=int, help="index of input device to use", default=0
)
parser.add_argument(
    "-m", "--manager", help="MARSH Manager IP addr", default="127.0.0.1"
)
parser.add_argument(
    "-M", "--manager-port", type=int, help="MARSH Manager port", default=24400
)
parser.add_argument(
    "-t",
    "--time-interval",
    type=float,
    help="interval between sent messages, in seconds",
    default=0.02,
)
...
if "hotas x" in device.get_name().lower():
    ...
    params["YAW_AXIS"] = 4.0
    params["YAW_REVERSED"] = 0.0
elif "Arduino Leonardo" in device.get_name():
